...
def vypisCisel(pocetCisel):
    """
    Po zadání parametru funkce vypíše daný počet čísel a určí, zdali je dělitelné dvojkou beze zbytku
    """
    for x in range(pocetCisel):
        if(x % 2 ==0):
            print(str(x) + " -> Cislo je beze zbytku delitelne dvema")
        else:
            print(x)
