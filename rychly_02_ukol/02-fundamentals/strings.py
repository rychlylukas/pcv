...
#ukol1
import datetime
print(datetime.date.strftime(datetime.datetime.strptime("12.2.2020", '%d.%m.%Y'), "%Y-%m-%d"))
#ukol2
import unidecode
souslovi='To je proměnná v Pythonu'

def prevod_na_promennou(souslovi):
    souslovi_promenna = souslovi.replace(" ", "_").lower()
    print(unidecode.unidecode(souslovi_promenna))
    souslovi_promenna = souslovi.title().replace(" ", "")
    print(unidecode.unidecode(souslovi_promenna))
prevod_na_promennou(souslovi)
#ukol3
import random
...
pocet_hesel=input('Zadejte pozadovany pocet hesel: ')

for i in range(int(pocet_hesel)):
    heslo=''
    for j in range(3):
        heslo += random.choice(abeceda.upper())
    for j in range(3):
        heslo += random.choice(abeceda)
    for j in range(1):
