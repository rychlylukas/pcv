...
#___slozitejsi zapis:___
# hundreds=[]
# for i in range(2001):
#     if i % 200 == 0:
#         hundreds.append(i)

#___jednoradkovy zapis:___
...
#___jednoradkovy zapis:___
asci=[random.choice(string.ascii_letters.upper()) for j in range(50)]
print(f"ascii = {asci}")
#cviceni c
del hundreds[-3:], hundreds[:3]
print(f"hundreds = {hundreds}")
#cviceni d
#set bere pouze unikátní
...
#cviceni a
persons+=(('Eva', 41, 'žena'), ('Jan', 25, 'muž'), ('Martin', 43, 'muž'), ('Ivana', 65, 'žena'))
print(persons)
...
i = 0
print("\nCoprehensions:\n")
ipersons = [item for item in persons if "i" in item[0].lower()]
ipersons.sort(key = sort_item, reverse=True)
for o in ipersons:
    print(f'{i};{o[0]};{o[1]};{o[2]}')
    i+=1
print("\nLambda:\n")
ipersons = list(filter(lambda item: "i" in item[0].lower(), persons))
ipersons.sort(key = sort_item, reverse=True)
i = 0
for o in ipersons:
    print(f'{i};{o[0]};{o[1]};{o[2]}')
